...
let stringsToNumbs = strNums.map((string) => Number(string));
console.log(stringsToNumbs);
// With the same numbers, find the sum of the even values
let sumEvens = stringsToNumbs.reduce((total, current) => {
  if (current % 2 === 0) {
...
let atxIdx = stringsToNumbs.findIndex((val, index) => val + index === 512);
...
let weatherArray = (arr) => {
  return arr
    .map((w) => w.weather_state_name)
...
let idealTemp = (arr) => {
  let weatherId = arr.find((w) => w.min_temp === 15.915);
  return weatherId.id;
};
